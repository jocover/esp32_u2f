...
static spinlock_t send_data_spinlock;
...
uint8_t CCID_Response_SendData(const uint8_t *buf, uint16_t len, uint8_t is_time_extension_request) {
  ...
  bulkin_state = len % CFG_TUD_CCID_EP_BUFSIZE == 0 ? CCID_STATE_DATA_IN_WITH_ZLP : CCID_STATE_DATA_IN;
...
uint8_t CCID_Init(void) {
  ...
  spinlock_initialize(&send_data_spinlock);
...
void CCID_TimeExtensionLoop(void* arg) {
...
void tud_ccid_tx_cb(uint8_t itf, uint16_t sent_bytes) {
